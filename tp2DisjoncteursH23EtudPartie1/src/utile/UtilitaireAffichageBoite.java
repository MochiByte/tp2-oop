...
public class UtilitaireAffichageBoite {
	...
	// Il se peut qu'on doive attendre avant de retourner une valeur.
	public static final int DUREE_ATTENTE_CREATION = 300;
	
	// On doit donner le temps au système d'intercepter un clic.
	public static final int DUREE_ATTENTE_CLIC = 100;
   ...
   /**
	...
	public static boolean optionMenuEstCliquee(){
		...
		GrilleGui.pause(DUREE_ATTENTE_CLIC);
	...
	public static void afficherBoite(Boite boite){
		...
		for(int i = 0; i < Boite.NB_LIGNES_MAX; i++){
			...
			for(int j = 0; j < Boite.NB_COLONNES; j++){
				...
				if(!boite.emplacementEstVide(j, i)){
	 ...
	 * type grille.  
	 * 
	 * AUX ÉTUDIANTS : Il n'est pas nécessaire de lire cette classe pour 
	...
	/**
		...
		public void setValeur(int y, int x, String valeur){
			...
			/*
			 * Comme c'est un Thread,  il se peut que la grille ne soit  
			 * pas encore créée alors on attend.
			 */
			if(grille[y][x]==null) {
		 ...
		 * Elle sert à mettre toutes les tailles à la même dimension.
		 * 
